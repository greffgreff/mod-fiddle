...
public class JigsawManager {
    public static void assembleJigsawStructure(DynamicRegistries dynamicRegistryManager, VillageConfig jigsawConfig, ChunkGenerator chunkGenerator, TemplateManager templateManager, BlockPos startPos, List<? super AbstractVillagePiece> structurePieces, Random random, boolean useHeightmap) {
        ...
        int previousWalkCount = 0;
        while(!assembler.availablePieces.isEmpty()) {
            Entry entry = assembler.availablePieces.removeFirst();
            String processedPiece = getPieceName(entry.piece.getJigsawPiece());

            if (previousWalkCount == 3)
                previousWalkCount = 0;
            if (processedPiece.contains("walk"))
                previousWalkCount += 1;

            ModFiddle.LOGGER.debug("Count: " + previousWalkCount + " Processed: " + processedPiece);

            assembler.tryPlacingChildren(entry.piece, entry.voxel, entry.boundsTop, entry.depth, previousWalkCount, processedPiece);
        }
    ...
    public static final class Assembler {
        ...
        public void tryPlacingChildren(AbstractVillagePiece piece, MutableObject<VoxelShape> voxelShape, int boundsTop, int depth, int previousWalkCount, String processedPiece) {
            // Get jigsaw piece
            ...
            for (Template.BlockInfo jigsawBlock : pieceJigsawBlocks) {
                ...
                if (depth != this.maxDepth) {
                    JigsawPiece generatedPiece = this.getPoolPiece(new ArrayList<>(jigsawBlockTargetPool.get().getShuffledPieces(random)), jigsawBlock, jigsawBlockTargetPos, pieceMinY, jigsawBlockPos, pieceVoxelShape, piece, depth, targetPieceBoundsTop, previousWalkCount, processedPiece);
                ...
                this.getPoolPiece(new ArrayList<>(jigsawBlockFallbackPool.get().getShuffledPieces(random)), jigsawBlock, jigsawBlockTargetPos, pieceMinY, jigsawBlockPos, pieceVoxelShape, piece, depth, targetPieceBoundsTop, previousWalkCount, processedPiece);
            }
        }

        private JigsawPiece getPoolPiece(List<JigsawPiece> candidatePieces, Template.BlockInfo jigsawBlock, BlockPos jigsawBlockTargetPos, int pieceMinY, BlockPos jigsawBlockPos, MutableObject<VoxelShape> pieceVoxelShape, AbstractVillagePiece piece, int depth, int targetPieceBoundsTop, int previousWalkCount, String processedPiece) {
            ...
            while (!candidatePieces.isEmpty()) {
                ...
                String pieceName = getPieceName(candidatePiece);

                ModFiddle.LOGGER.debug(pieceName);

                if (!(previousWalkCount == 0 && !pieceName.contains("pillarhead") && !processedPiece.contains("pillarhead")) ||
                    !(previousWalkCount < 3 && !pieceName.contains("walk"))) {
